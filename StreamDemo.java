package com.dnapass.training.java.stream;
...
import com.dnapass.training.java.transaction.*;
...
// TODO Auto-generated method stub
		...
		Stream<String> lines = Files.lines(Paths.get("C:\\Users\\r-praveenkumar\\Sample.txt"),
...
// TODO Auto-generated method stub
		...
		transactionIds = transactions.stream().filter(t -> t.getType() == ProductType.GROCERIES)
				.sorted(comparing(Transaction::getAmount).reversed()).map(Transaction::getId).collect(toList());
		System.out.println(transactionIds);
		...
		transactions.stream().filter(t -> t.getType() == ProductType.GROCERIES).findAny()
		...
		String[] arrayOfWords = { "Java abstraction inheritance encapsulation functional",
				"Jjva abstraction inheritance encapsulation functional" };
		Stream<String> streamOfWords = Arrays.stream(arrayOfWords);
		streamOfWords.map(line -> line.split("\\s+")).flatMap(Arrays::stream).distinct().forEach(System.out::println);
		List<Integer> expensiveTransactionIds = transactions.stream().filter(t -> t.getAmount() > 1000)
				.map(Transaction::getId).collect(toList());
		System.out.println(expensiveTransactionIds);
		Set<String> cities = transactions.stream().filter(t -> t.getAmount() > 1000).map(Transaction::getCity)
				.collect(toSet());
		System.out.println(cities);
		cities = transactions.stream().filter(t -> t.getAmount() > 1000).map(Transaction::getCity)
		...
		Optional<Transaction> highestTransaction = transactions.stream()
				.collect(maxBy(comparing(Transaction::getAmount)));
...
//grouping
		...
		Map<Boolean, List<Transaction>> partitionedTransactions = transactions.stream()
				.collect(partitioningBy(t -> t.getAmount() > 1000));
		System.out.println(partitionedTransactions);
		Map<String, Double> cityToSum = transactions.stream()
				.collect(groupingBy(Transaction::getCity, summingDouble(Transaction::getAmount)));
		System.out.println(cityToSum);
		Map<String, Optional<Transaction>> cityToHighestTransaction = transactions.stream()
				.collect(groupingBy(Transaction::getCity, maxBy(comparing(Transaction::getAmount))));
		System.out.println(cityToSum);
// Map<String,Double> cityToCurrencyToAverage = transactions.stream().collect(groupingBy(Transaction::getCity,groupingBy(Transaction::getCurrency,averagingDouble(Transaction::getAmount))));
		...
		people.parallelStream().filter((it) -> it.getAge() >= 21)
				.forEach((it) -> System.out.println(" " + it.getFirstName() + Thread.currentThread()));
